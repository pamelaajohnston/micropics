...
if __name__ == "__main__":
    ...
    create_model = False
    run_models = False
    lr = 0.0002 # the learning rate for the GAN, sometimes smaller
    ...
    parser.add_argument("-a", "--matrixEntry", help="the entry in the test matrix to perform (pixels), default=0")
    ...
    parameters_to_change_orig = [
        ["base1_224",                  True,   True,   True,   True, 224, 1, "trichome_on_top", "hp_filter"],
        #### These were not generating a model for runs<5, probably because they were compensating for something....(Present Pam October 2023)
    ...
    if args.matrixEntry:
        parameters_to_change = [ parameters_to_change_orig[int(args.matrixEntry)], ]
        print("Parameters running: ")
    ...
    for selection in parameters_to_change:
        ...
        if create_model:
            ...
            #print(image_shape)
            # An extra step for learning rate
            modelname = str(model_name)
            if "no_trichome" in modelname:
                lr = 0.00002
            
            # define the models
            d_model = m.define_discriminator(image_shape, lr=lr)
            g_model = m.define_generator(image_shape)
            # define the composite model
            gan_model = m.define_gan(g_model, d_model, image_shape, lr=lr)
            # train model
